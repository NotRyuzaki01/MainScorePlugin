...
public class TeleportCommandHandler implements CommandExecutor, TabCompleter {
    ...
    public TeleportCommandHandler(MainScorePlugin plugin) {
        this.plugin = plugin;
    }
        ...
        BukkitRunnable countdown = new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (currentLoc.distanceSquared(startLoc) > 0.01) {
                    ...
                    TextComponent cancelMsg = new TextComponent("Teleport cancelled because ");
                    cancelMsg.setColor(ChatColor.RED);
                    TextComponent mover = new TextComponent(teleporting.getName());
                    mover.setColor(ChatColor.of("#0094FF"));
                    TextComponent moved = new TextComponent(" moved!");
                    moved.setColor(ChatColor.RED);
                    cancelMsg.addExtra(mover);
                    cancelMsg.addExtra(moved);

                    target.spigot().sendMessage(ChatMessageType.ACTION_BAR, cancelMsg);
                    target.sendMessage(cancelMsg);
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        switch (name) {
            ...
            case "tpahere": {
                ...
                if (!plugin.isTpaEnabled(target.getUniqueId())) {
                    player.sendMessage("§cThat player has TPA requests disabled.");
                    player.spigot().sendMessage(ChatMessageType.ACTION_BAR, new TextComponent("§cThat player has TPA requests disabled."));
                    player.playSound(player.getLocation(), Sound.ENTITY_VILLAGER_NO, 1f, 1f);
                    return true;
                }
                ...
                TextComponent part2 = new TextComponent(" sent a ");
                part2.setColor(ChatColor.WHITE);
                full.addExtra(part2);

                TextComponent requestType = new TextComponent(name);
                requestType.setColor(ChatColor.of("#0094FF"));
                full.addExtra(requestType);

                TextComponent part3 = new TextComponent(" request. Use ");
                part3.setColor(ChatColor.WHITE);
                full.addExtra(part3);

                TextComponent tpaccept = new TextComponent("/tpaccept");
                tpaccept.setColor(ChatColor.GREEN);
                full.addExtra(tpaccept);

                TextComponent part4 = new TextComponent(" or ");
                part4.setColor(ChatColor.WHITE);
                full.addExtra(part4);

                TextComponent tpdeny = new TextComponent("/tpdeny");
                tpdeny.setColor(ChatColor.RED);
                full.addExtra(tpdeny);
            ...
            case "tpaccept": {
                ...
                if (Combat.isInCombat(player) || Combat.isInCombat(requester)) {
                    ...
                    TextComponent cancelMsg = new TextComponent("Teleport cancelled — ");
                    cancelMsg.setColor(ChatColor.RED);
                    TextComponent enemyName = new TextComponent(player.getName());
                    enemyName.setColor(ChatColor.of("#0094FF"));
                    cancelMsg.addExtra(enemyName);
                    cancelMsg.addExtra(new TextComponent(ChatColor.RED + " or you is in combat!"));

                    requester.spigot().sendMessage(ChatMessageType.CHAT, cancelMsg);
                    requester.spigot().sendMessage(ChatMessageType.ACTION_BAR, cancelMsg);
